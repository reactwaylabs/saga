...
//             this.buildError(
//                 this.getState.name,
//                 "As state behind the store is mutable, store-specific methods must be used (get, has, etc.)."
//             )
//         );
//     }
...
//             // And queue its retrieval

//             return newItem;
//         }

//         // If key exists, we are sure value is not undefined.
...
//      * @param noCache Should cached item be re-fetched from the server.
//      */
//     protected getItem(key: string, noCache: boolean): Item<TValue> {
...
//         const state = prev || this.getInitialState();
...
//         return state.withMutations(resultMap => {
//             // remove any old keys that are not in new keys list or are no longer in the cache
//             if (resultMap.size > 0) {
